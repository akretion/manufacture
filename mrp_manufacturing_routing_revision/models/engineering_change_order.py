...
class EngineeringChangeOrder(models.Model):
    ...
    def _compute_plan_ids(self):
        for rec in self:
            if rec.manufacturing_routing_id:
                plans = rec.manufacturing_routing_id.product_ids.plan_ids
    ...
    def action_confirm(self):
        ...
        if self.manufacturing_routing_id:
            ...
            if self.manufacturing_routing_id.current_revision_id:
                ...
                vals["manufacturing_routing_id"] = self.manufacturing_routing_id.id
                self.env["manufacturing.routing.revision"].create(vals)
        return res
    ...
    def action_view_new_manufacturing_routing_revision(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id(
            "mrp_manufacturing_routing_revision.action_manufacturing_routing_revision"
        )
        if len(revisions) > 1:
            ...
            form_view = [
                (
                    self.env.ref(
                        "mrp_manufacturing_routing_revision.manufacturing_routing_revision_form_view"
                    ).id,
                    "form",
                )
            ]
